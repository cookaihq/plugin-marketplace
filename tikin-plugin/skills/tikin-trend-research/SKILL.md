...
version: 0.3.0
description: v0.3.0｜Discover trends via tikin — viral content, rising hashtags, hot sounds, and ranking boards across supported platforms. Use when the user asks what is trending, wants a regional or niche trend report, or provides a supported URL as trend context.
---
...
tikin_run() {
  uv run --project "${TIKIN_SETUP_DIR}" "${TIKIN_SETUP_DIR}/scripts/tikin-config" \
    --skill tikin-trend-research run -- "$@"
}
```
...
`$PWD/.env.tikin-trend-research` → `$PWD/.env.local` → `$PWD/.env` → the existing
