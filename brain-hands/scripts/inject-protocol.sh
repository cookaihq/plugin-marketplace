...
CTX+="- Model routing: the hands agent definition already pins its own executor model, so dispatching hands WITHOUT a model parameter runs that pinned model and does not burn brain-tier quota - omitting the parameter is the correct default for hands. Only pass a cheaper override on hands for mechanical bulk work, and name that reason when you do. Any OTHER execution- or review-type subagent you spawn (including ones a skill tells you to spawn) has no pinned model and must be given an explicit model parameter - unspecified there inherits YOURS and burns brain-tier quota. Never use fork-type subagents for such work; they always inherit the parent model.\n"
